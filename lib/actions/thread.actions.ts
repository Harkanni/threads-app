...
export async function likeThread(threadId: string, path: string) {
   ...
   try {
      ...
      let post = await Thread.findById(threadId)

      // If post does'nt exist
      if (!post) {
         console.log("Thread not found.");
         throw new Error(`Thread not found`);
      }

      // if theres no like property exists
      if (!Array.isArray(post.likes)) {
         await Thread.updateMany(
            { likes: { $exists: false } }, // Condition to match documents without the "likes" field
            { $set: { likes: [] } }, // Add the "likes" field with an initial empty array
         );
      }

      const index = post.likes.findIndex((id: any) => id == threadId)

      index === -1 ? post.likes.push(threadId) : post.likes.splice(index, 1)

      const updatedPost = await post.save(); // Save the updated document
